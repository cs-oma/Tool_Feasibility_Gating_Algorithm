...
# ------------------------------------------------------------------------------
#  File: Core/MultiAgentSimulator.py
#  Purpose: Multi-agent discrete-event simulation on a shared single-server queue.
...
class MultiAgentSimulator:
    """Shared-queue simulator with multiple independent arrival streams.

    Each agent i has its own interarrival model. Arrivals are merged in the event
    calendar. Tasks are tagged with agent_id_i32 and routed to agent-specific
    decision policies via a composite Scheduling_Policy.

    The service discipline is non-preemptive, single server. Task selection is
    handled by policy.Select_Task (default FCFS, optionally priority-first).
    """

    def __init__(
        self,
        cfg_simulation_config: Simulation_Config,
        interarrival_models_list: List[Interarrival_Model],
        ...
        if not interarrival_models_list:
            raise ValueError("interarrival_models_list must be non-empty.")

        self.cfg_simulation_config = cfg_simulation_config
        self.interarrival_models_list = interarrival_models_list
        ...
        self._arrivals_by_agent_list_i32: List[int] = [0 for _ in interarrival_models_list]

        self._in_system_list_task: List[Task] = []

    def _Make_Task(self, agent_id_i32: int, arrival_time_f64: float) -> Task:
    ...
    def Initialize(self) -> None:
        ...
        for agent_id_i32, model in enumerate(self.interarrival_models_list):
            first_inter_f64 = float(model.Sample(self.rng_rng))
            first_arrival_time_f64 = 0.0 + first_inter_f64
            self.calendar_event_calendar.Schedule(
                time_f64=first_arrival_time_f64,
                event_type_event_type=Event_Type.ARRIVAL,
                payload_any=int(agent_id_i32),
            )
    ...
    def Run(self) -> dict:
        ...
        while True:
            ...
            if ev_event_opt.event_type == Event_Type.ARRIVAL:
                self._Handle_Arrival(ev_event_opt)
        ...
        for tk in pending_list_task:
            obs = getattr(self.policy_scheduling_policy, "Observe_Task_Outcome", None)
            if callable(obs):
                obs(tk)

        return self.metrics_metrics_collector.Aggregate()

    def _Schedule_Next_Arrival(self, agent_id_i32: int) -> None:
        ...
        model = self.interarrival_models_list[int(agent_id_i32)]
        inter_f64 = float(model.Sample(self.rng_rng))
    ...
    def _Handle_Arrival(self, ev_event: Event) -> None:
        agent_id_i32 = int(ev_event.payload)

        self._arrivals_count_i32 += 1
        self._arrivals_by_agent_list_i32[agent_id_i32] += 1

        task_ = self._Make_Task(agent_id_i32, self.now_f64)
        self._in_system_list_task.append(task_)

        self.queue_deque_task.append(task_)

        # Optional: pre-decide mode at arrival (keeps parity with original TFG tracing).
        ...
        self._Schedule_Next_Arrival(agent_id_i32)
